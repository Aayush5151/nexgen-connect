...
/**
 * SocialChips. Placeholder social icon row linking to `#` until real
 * handles are created. Every chip has a proper aria-label so screen
 * readers do not announce "# link". The set is intentionally small —
 * the full official logo lockups live in /brand for partnerships.
 */
      ...
      {SOCIALS.map((s) => (
        <li key={s.label}>
          <Link
            ...
            aria-label={s.label}
            className={cn(
              "group inline-flex items-center justify-center rounded-full border transition-colors duration-150",
              tone === "default"
