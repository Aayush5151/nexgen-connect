        ...
        <ScrollReveal>
          <div className="text-center">
            ...
            <h2 className="mt-5 text-3xl font-black tracking-[-0.02em] text-[#E8E8ED] sm:text-4xl lg:text-5xl">
              Built <span className="text-[#E8E8ED] italic">Different</span>
            </h2>
            <p className="mx-auto mt-4 max-w-xl text-base font-medium text-[#9CA3AF] md:text-lg">
              ...
              <span className="font-semibold text-[#E8E8ED]">A verified, intentional network.</span>
            </p>
          </div>
        </ScrollReveal>
        ...
        <div className="mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((feature, i) => (
            <ScrollReveal key={feature.title} delay={i * 0.08} variant="scaleUp">
              <GlowCard glowColor={feature.glowColor} className="h-full border-white/[0.06] bg-[#141416]">
                <div className="flex h-full flex-col p-7">
                  {/* Icon container */}
                  <div className="flex items-center justify-between">
                    <div className="inline-flex h-14 w-14 items-center justify-center rounded-xl bg-[#1E1E22]">
                      <div className={`flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br ${feature.gradient} shadow-sm`}>
                        <feature.icon className="h-5 w-5 text-white" />
                      </div>
                    </div>
                    {feature.badge && (
                      <span className="rounded-full bg-[#2B3FC7]/10 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-[#2B3FC7]">
                        {feature.badge}
                      </span>
                    )}
                  </div>
                  <h3 className="mt-5 text-lg font-bold text-[#E8E8ED]">{feature.title}</h3>
                  <p className="mt-3 flex-1 text-sm font-medium leading-relaxed text-[#9CA3AF]">
                    {feature.description}
                  </p>
                </div>
              </GlowCard>
