...
const BOILERPLATE_EN = `NexGen Connect is a mobile app that matches Indian students moving abroad with verified classmates inside a corridor — defined as their home city in India, their destination city, and their intake month. Every member is verified through three independent checks: phone OTP via MSG91, Aadhaar consent via DigiLocker (a one-way signed token — the 12-digit Aadhaar number itself is never stored), and a human-reviewed admit letter (within 48 hours of submission). Group DMs only open once 60 verified students share the same corridor; below that threshold we tell users the group isn't real yet. Inside an unlocked corridor, university-specific subgroups auto-spawn whenever 20 or more classmates verify at the same school. The app launches with two corridors: India to Ireland in September 2026, connecting students headed to University College Dublin, Trinity College Dublin, University College Cork, and other HEIs; and India to Germany in October 2026, connecting students headed to the Technical University of Munich, LMU Munich, RWTH Aachen, Humboldt University of Berlin, and other HEIs. NexGen Connect is students-only, with a free tier that covers matching, verification, group DMs, uni subgroups, and the pre-flight countdown. A one-time Premium unlock at rupees 1,499 (no subscription, no renewal) adds priority matching (first seat when corridors unlock), group-apply apartment tooling (a bundled 3 to 6 student PBSA application in one signature flow), a read-only Parent view (group size, verification status, arrival time — never chats), and a 30-minute human call within 24 hours. No immigration agents. No recruiters. No ads. No data sale.`;

const BOILERPLATE_SHORT = `NexGen Connect is a verified, students-only mobile app that matches Indian students with classmates from their home city, going to the same destination, in the same intake month. DMs unlock when 60 verified students share a corridor. Free to use. Premium one-time unlock for priority matching, group-apply housing, and parent peace of mind. Two launch corridors in 2026: India to Ireland (September) and India to Germany (October).`;
